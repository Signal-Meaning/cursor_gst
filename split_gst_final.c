...
int main(int argc, char *argv[]) {
    GstElement *pipeline, *source, *demux, *queue1, *queue2, *queue3;
    ...
    if (!pipeline || !source || !demux || !queue1 || !queue2 || !queue3 ||
    ...
    // Build pipeline
    gst_bin_add_many(GST_BIN(pipeline), source, demux, queue1, queue2, queue3,
                     audio_convert1, audio_convert2, audio_enc1, audio_enc2,
    ...
    // Link demux to queues (this will be done in pad-added callback)
...
static void on_pad_added(GstElement *src, GstPad *new_pad, gpointer user_data) {
    ...
    if (g_str_has_prefix(name, "audio/")) {
        g_print("Linking audio pad to queue1 and queue2\n");
        // Link demux audio pad to both queue1 and queue2
        GstPad *queue1_sink = gst_element_get_static_pad(queue1, "sink");
        GstPad *queue2_sink = gst_element_get_static_pad(queue2, "sink");
        
        if (gst_pad_is_linked(queue1_sink) == FALSE) {
            gst_pad_link(new_pad, queue1_sink);
        }
        if (gst_pad_is_linked(queue2_sink) == FALSE) {
            gst_pad_link(new_pad, queue2_sink);
        }
